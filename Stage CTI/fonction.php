...
{
    global $pdo;
    $query = 'SELECT * FROM entretien WHERE `Nom_Candidat`=:nom or `Prenom_Candidat`=:prenom or `Poste_id`=:pouvoir or `Type` =:entretienType
    ...
    $prep->bindValue(':pouvoir', $pouvoir, PDO::PARAM_INT);
...
function AfficheAvancementParMois($numAgent,$nomCanevas)
{
    global $pdo;
    $query = 'SELECT DISTINCT mois, `avancementCanevas` FROM `P2I` WHERE numAgent = :numAgent AND nom_canevas = ":nomCanevas"';
    $prep = $pdo->prepare($query);
    $prep->bindValue(':numAgent', $numAgent, PDO::PARAM_INT);
    $prep->bindValue(':nomCanevas', $nomCanevas, PDO::PARAM_STR);
    $prep->execute();
